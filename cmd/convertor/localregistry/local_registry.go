...
// REGISTRY
// LocalRegistry is a fully flushed local registry implementation. Its meant
// primarily for testing purposes but can be used to also store converted
// images into a local store. The imeplementation supports a combination of
// in memory and local storage, where the in memory storage is used for pushes
// and overrides when IsTestingRegistry=true.
...
type RegistryOptions struct {
	...
	IsTestingRegistry         bool   // Specifies if the registry is used for testing purposes. This determines if pushes are written to the filesystem.
}

type internalRegistry map[string]*RepoStore

// NewLocalRegistry creates a new local registry instance
func NewLocalRegistry(ctx context.Context, opts RegistryOptions) (*LocalRegistry, error) {
	TestRegistry := LocalRegistry{
		opts:             opts,
		internalRegistry: make(internalRegistry),
	}

	if !opts.IsTestingRegistry && opts.InmemoryOnly {
		return nil, errors.New("InmemoryOnly can only be used with IsTestingRegistry")
	}
...
func (r *LocalRegistry) Resolve(ctx context.Context, ref string) (v1.Descriptor, error) {
...
func (r *LocalRegistry) Fetch(ctx context.Context, repository string, descriptor v1.Descriptor) (io.ReadCloser, error) {
...
func (r *LocalRegistry) Push(ctx context.Context, repository string, tag string, descriptor v1.Descriptor, content []byte) error {
...
func (r *LocalRegistry) Exists(ctx context.Context, repository string, tag string, desc v1.Descriptor) (bool, error) {
	repo, ok := r.internalRegistry[repository]
	if !ok {
		return false, nil
	}
